...
df = pd.read_csv("./data/(과제)UK_Accident.csv", parse_dates=['Date', 'Time'])
df.columns
...
#따라서 null 값은 데이터에 있는 행 수에 비해 매우 적으므로 행을 삭제할 수 있습니다.
...
#중복된 행의 총 개수는 34155이므로 이 행을 삭제할 수 있습니다.

df.drop_duplicates(inplace=True)
print("No. of rows remaining: ", df.shape[0])

#다음은 중복된 행을 삭제한 후의 데이터 모양입니다.
...
print(corr_features)
#이는 상관 관계가 80% 이상인 단 하나의 열이므로 다음 열을 삭제할 수 있습니다.
...
for col in cnt_lst1:
  cnt_plot(df, col)
#대부분의 사고에서는 도로는 1차선이었다.
#교차점은 통제되지 않은 교차점이거나 전혀 교차점이 없었습니다.
#현장에서 50m 이내에는 사람이 통제하는 횡단보도가 없었습니다.
#날씨가 건조했어요.
...
for col in cnt_lst2:
  cnt_plot(df, col)
#대부분의 사고에서는 현장으로부터 50m 이내에는 물리적인 횡단이 없었습니다.
#낮에 일어난 일이라 가시성은 괜찮았습니다.
#날씨는 바람이 많이 불지 않고 괜찮았습니다.
#특별한 조건이나 차도에 문제는 없었습니다.
...
#기능은 단지 개수에 불과하므로 기능을 삭제할 수 있습니다(다음 기능은 모델에 가치를 추가하지 않습니다).
...
plt.show()
#기능 사고 심각도는 "차량 수", "사고 수" 및 "속도 제한"과 같은 기능과 높은 상관 관계가 있습니다.
...
#따라서 범주형 변수의 값은 2에서 9까지이므로 LabelEncoder를 사용할 수 있습니다.
